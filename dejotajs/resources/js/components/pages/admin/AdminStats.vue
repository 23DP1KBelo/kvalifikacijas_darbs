    ...
    <v-row justify="center" class="mb-10" dense>
      ...
      <v-col cols="12" sm="6" md="3">
        <v-card class="pa-6 text-center elevation-6 group-card">
          ...
          <div class="text-subtitle-1 text-gray-100">Kopējais deju kolektīvu skaits</div>
        </v-card>
      </v-col>
    </v-row>
...
export default {
  ...
  methods: {
    async fetchGroupsAndRenderCharts() {
        try {
          const res1 = await fetch('/api/admin/stats', { withCredentials: true });
          const data1 = await res1.json();
          this.groups = Array.isArray(data1.data) ? data1.data : [];

          const res2 = await fetch('/api/admin/stats-dancers', { withCredentials: true });
          const data2 = await res2.json();
          this.dancerGroups = Array.isArray(data2.data) ? data2.data : [];
    ...
    renderCharts() {
      ...
      new Chart(ctxStatus, {
        type: 'bar',
        data: {
          labels: ['Kopā', 'Gaida apstiprinājumu', 'Apstiprināti', 'Noraidīti'],
          datasets: [{
      ...
      const labels = this.dancerGroups.map(g => g.name);
      const dancersCount = this.dancerGroups.map(g => g.dancers.length);
